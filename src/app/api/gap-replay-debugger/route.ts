...
// AI gap: Agent-Assisted Replay Debugger (noCodeAIAgency)
...
async function callLLM(systemPrompt: string, userMsg: string, maxTokens = 1400) {
  ...
  const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + OPENROUTER_API_KEY, 'Content-Type': 'application/json', 'HTTP-Referer': 'http://localhost:3000', 'X-Title': 'noCodeAIAgency Gap replay-debugger' },
...
export async function POST(req: NextRequest) {
  try {
    ...
    const sys = "You summarize replay sessions of agent runs. Identify failure points, suggest fixes, and surface useful patterns.";
    const user = `Body: ${JSON.stringify(body).slice(0, 4000)}`;
    const out = await callLLM(sys, user);
    gapFeatures.push({ at: new Date().toISOString(), slug: 'replay-debugger', payload: { keys: Object.keys(body) } });
...
export async function GET() {
  return NextResponse.json({ feature: 'replay-debugger', events: gapFeatures.filter(r => r.slug === 'replay-debugger').length });
}

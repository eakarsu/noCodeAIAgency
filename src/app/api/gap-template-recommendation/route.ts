...
// AI gap: AI Template Recommendation (noCodeAIAgency)
...
async function callLLM(systemPrompt: string, userMsg: string, maxTokens = 1400) {
  ...
  const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + OPENROUTER_API_KEY, 'Content-Type': 'application/json', 'HTTP-Referer': 'http://localhost:3000', 'X-Title': 'noCodeAIAgency Gap template-recommendation' },
...
export async function POST(req: NextRequest) {
  try {
    ...
    const sys = "You are a template-search RAG assistant for a no-code AI platform. Given user requirements, recommend the 3 best-fit templates with rationale.";
    const user = `Body: ${JSON.stringify(body).slice(0, 4000)}`;
    const out = await callLLM(sys, user);
    gapFeatures.push({ at: new Date().toISOString(), slug: 'template-recommendation', payload: { keys: Object.keys(body) } });
...
export async function GET() {
  return NextResponse.json({ feature: 'template-recommendation', events: gapFeatures.filter(r => r.slug === 'template-recommendation').length });
}
